...
import * as BarcodeScanner from '@qr_code_scanner/js/barcode_scanner';
...
//            this.packagesEnabled = await user.hasGroup('stock.group_tracking_lot');
    ...
    removeDemoMessage() {
        ...
        const params = {
            ...
            confirm: () => {
                this.rpc('/qr_code_scanner/rid_of_message_demo_barcodes');
                location.reload();
            },
            cancel: () => {},
    ...
    async _onBarcodeScanned(barcode) {
        const res = await this.rpc('/qr_code_scanner/scan_from_main_menu', { barcode });
        console.log(res)
        if (Number.isInteger(res)) {
            this.notificationService.add("Record Added successfully!", { type: 'success' });
            return;
        }
        this.notificationService.add(res.warning, { type: 'danger' });
    }
}
MainMenu.template = 'qr_code_scanner.MainMenu';

registry.category('actions').add('qr_barcode_main_menu', MainMenu);
